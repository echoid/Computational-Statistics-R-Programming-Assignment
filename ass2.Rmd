...
```{r}
mixture.EM <- function(X, w.init, mu.init,sigma.init, epsilon=1e-6, max.iter=100) {
  ...
  while((criteria > epsilon) & (n.iter <= max.iter)){
  ...
  return(list(w.curr=w.curr, mu.curr=mu.curr, sigma.curr = sigma.curr,criteria=e_list))
}
```
...
```{r}
EM <- mixture.EM(c(I), w.init=c(0.3,0.3, 0.4), mu.init=c(0, 0.5, 1),sigma.init=c(0.1, 0.1, 0.1), epsilon=1e-6, max.iter=100)
```
